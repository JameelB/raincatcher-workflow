...
/**
 * Initialize a subscriber for reading workflows.
 *
...
module.exports = function readWorkflowSubscriber(workflowEntityTopics, workflowClient) {
  ...
  return function handleReadWorkflowsTopic(parameters) {
    ...
    workflowClient.read(parameters.id)
    .then(function(workflow) {
      workflow.syncStatus = SyncEvents.getSyncStatus(workflow.id);
      debug('Workflow %O',  workflow);
