...
var workflowStepSubscribers;
var workflowSyncEventSubscribers;
...
module.exports = {
  ...
  init: function(mediator, workflowClient) {
    ...
    workflowSyncEventSubscribers = new MediatorTopicUtility(mediator);
    workflowSyncEventSubscribers.prefix(CONSTANTS.SYNC_TOPIC_PREFIX).entity(CONSTANTS.WORKFLOW_ENTITY_NAME);

    _.each(CONSTANTS.SYNC_EVENT_TOPICS, function(syncEventTopic) {
      workflowSyncEventSubscribers.on(syncEventTopic, topicHandlers.syncEvent());
    });
  ...
  tearDown: function() {
    ...
    if (workflowSyncEventSubscribers) {
      workflowSyncEventSubscribers.unsubscribeAll();
    }
  }
};
